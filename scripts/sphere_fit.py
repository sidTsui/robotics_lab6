...
def filtered(unfiltered, filter_gain, output, xOutput, yOutput, zOutput, rOutput):
	...
	xOutput = filter_gain.get('xc') * xInput + (1 - filter_gain.get('xc')) * xOutput
	yOutput = filter_gain.get('yc') * yInput + (1 - filter_gain.get('yc')) * yOutput
	zOutput = filter_gain.get('zc') * zInput + (1 - filter_gain.get('zc')) * zOutput
	rOutput = filter_gain.get('radius') * rInput + (1 - filter_gain.get('radius')) * rOutput
